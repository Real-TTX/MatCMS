...
/// <summary>
/// The instance-side second factor: wraps the shared, dependency-free <see cref="TwoFactor"/> algorithm
/// with the two things it deliberately leaves out — storing the TOTP secret ENCRYPTED at rest
/// (DataProtection, the same protection the cloud token gets) and persisting enrolment / recovery state
/// on the <see cref="User"/> row. Enrolment writes nothing until the user proves they can generate a
/// code, so an abandoned setup leaves no trace.
/// </summary>
public class TwoFactorService
{
    ...
    // The issuer shown in the authenticator app. A STATIC label, not the (renamable) site name, so a
    // later rename does not orphan already-enrolled entries — the secret is what matters, not the label.
    private const string Issuer = "MatCMS";
    ...
    {
        _db = db;
        _protector = dp.CreateProtector("MatCMS.TwoFactor.Secret.v1");
        _cache = cache;
    }
    ...
    /// cross-instance restore) only disables the TOTP branch — recovery codes are key-independent hashes
    /// and must still work, or a legitimate admin is hard-locked out.</summary>
    ...
    private async Task<bool> TryConsumeRecoveryCodeAsync(User user, string code)
    {
        ...
        var remaining = (user.RecoveryCodes ?? "")
            ...
            .ToList();
        // The stored value is a SHA-256 hash of a high-entropy random code, so an ordinal compare here
        // leaks nothing useful; single use is enforced by removing the matched hash.
