...
/// <summary>
/// Self-service two-factor setup for the LOGGED-IN user: scan the QR, confirm a code, keep the recovery
/// codes. An admin can only <em>reset</em> another account (Users/Edit), never enrol it — the secret is
/// confirmed by the person holding the device, so it is never set on someone's behalf.
/// </summary>
public class TwoFactorModel : PageModel
{
    ...
    /// <summary>True when the "2FA required" policy sent the user here (shown as a notice).</summary>
    ...
    /// POST so nothing touches the DB until the code is proven. It is displayed to the user anyway
    /// (QR + manual key), so a hidden copy is no extra exposure.</summary>
    ...
    public async Task<IActionResult> OnPostEnableAsync()
    {
        ...
        if (recovery is null)
        {
            // Wrong code: re-render with the SAME secret so the user does not have to rescan.
            Enrolled = false;
            OtpauthUri = _twoFactor.OtpauthUri(user, PendingSecret);
